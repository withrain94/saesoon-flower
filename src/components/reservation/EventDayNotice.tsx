...
import { formatDeadline, getEventCopy, getEventDeadline } from "@/lib/events";
...
export default function EventDayNotice({
  ...
  return (
    <div>
      ...
      <p className="mt-0.5 text-[14px] font-bold text-brand-dark">{formatDateLong(event.date, t)}</p>
      <ul className="mt-2 space-y-1 text-[13.5px] leading-relaxed text-body">
        ...
        {deadline && (
          <li className="flex gap-1.5 font-semibold text-ink">
            ...
            <span>{t.eventBanner.deadline(formatDeadline(deadline, t))}</span>
          </li>
        )}
      </ul>
    </div>
