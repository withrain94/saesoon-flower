...
├─ lib/                      순수 함수 (date, time, events, format, selection, units, reservationRequest, documents)
├─ data/                     정적 데이터 (products, shop, reservationOptions, events)
...
1. **의존 방향은 한쪽으로만**: `types → data → lib → hooks → components/ui → components/(layout|reservation) → app`
...
5. 매장 정보(계좌·네이버 링크·소개 사이트 링크 `siteUrl`/`siteLinksByCategory`)는 `data/shop.ts`, 상품(꽃다발·꽃바구니·호접난)은 `data/products.ts`에서만 수정한다.
   종류 이름을 화면 문구에 직접 나열하지 말고 `categoryNames` / `productCategories`를 쓴다.
   새 상품 종류를 추가할 땐 아래 순서로 (타입 오류가 빠진 곳을 알려준다):
   `types`의 `ProductCategoryId` → `products.ts` (`colorChoice`, 필요하면 `notice`) → `shop.ts`의 네이버 링크 → `reservationOptions.ts`의 `messageOptionsByCategory`.
   블랙보드 문구 선택지(예: 호접난 "알아서 써주세요" 등)는 `reservationOptions.ts`의 `blackboardPresetsByCategory`에서만 수정한다. ("직접 입력"은 자동으로 붙음)
