...
`NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY`(Config) · `SUPABASE_SECRET_KEY`(Secret) · `ADMIN_EMAILS`(Config) · `TELEGRAM_BOT_TOKEN`(Secret) · `TELEGRAM_CHAT_ID`(Config) · `NOTION_TOKEN`(Secret) · `NOTION_PARENT_PAGE_ID`(Config) · `NEXT_PUBLIC_KAKAO_JS_KEY`(Config, 카카오 버튼용). `.env.local`에는 9개 모두 값 있음(카카오 키 2026-09-15 추가). Vercel 카카오 키는 사용자가 넣는 중.
...
- **완료 화면 카카오톡으로 나에게 보내기·복사 (배포 완료 1a41e12)** — Vercel에 `NEXT_PUBLIC_KAKAO_JS_KEY`(Config)를 넣고 재배포해야 카카오 버튼이 보임. 없으면 복사 버튼만. ✅ 카카오 개발자 앱(이름 "새순") 만들고 JavaScript SDK 도메인에 배포 주소·localhost:3000 등록(사용자, 2026-09-15). ✅ `.env.local`에 키 넣고 로컬에서 카카오 공유 창 열림 확인(도메인 오류 없이 카카오 로그인 화면까지). ⏳ Vercel에 키 넣기 → 재배포 → 휴대폰에서 버튼 확인.
