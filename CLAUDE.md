...
- **견적서·거래명세표: 이메일 없애고 PDF 다운로드로 (2026-09-15, 사용자 요청, 배포 완료 27255bf)** — Gmail 설정은 안 하기로 함. 이메일 칸·자동 발송·관리자 "다시 보내기"·nodemailer 삭제. 로컬에서 PDF 생성(한글·직인) 확인. 실제 사이트 경로 응답(잘못된 요청 400·없는 예약 404) 확인. **서류를 요청한 실제 예약으로 휴대폰 다운로드(글꼴·직인 포함)는 아직 확인 못 함** → 첫 서류 요청 예약 때 관리자 상세에서 눌러 확인.
