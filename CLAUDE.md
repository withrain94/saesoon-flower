...
| 새 예약 알림 | **텔레그램**(2026-09-15 사용자 선택). 예약 저장 성공 뒤 매장 텔레그램으로: 접수번호·일시(특별한 날)·상품·금액·결제·**예약자 이름·연락처**(사용자 결정)·호접난 배송·토퍼/서류 여부·외국어 신청·관리자 링크. 실패해도 예약은 저장됨. 환경변수 `TELEGRAM_BOT_TOKEN`(비밀)·`TELEGRAM_CHAT_ID`(쉼표로 여러 곳). 처리방침 위탁에 Telegram 추가. 봇 **@saesoon_reservation_bot**, 받는 곳 = 사장님(Kim R) 채팅 — `.env.local`·Vercel(Production)에 둘 다 넣음(2026-09-15), 채팅 번호 8138762410. 배포 4b5a1fa → 실제 사이트 테스트 예약 C8B26952 저장(취소 처리). **텔레그램 도착은 사용자 확인 대기**. 직원 추가 = 봇에게 말 건 뒤 getUpdates로 채팅 번호 찾아 쉼표로 추가 | `server/notify.ts`, `lib/reservationNotice.ts` |
