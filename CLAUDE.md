...
## 5. 지금 상태 (2026-09-15 21:05 기준)

| 부분 | 상태 |
|---|---|
| 손님 예약 페이지 | ✅ 실제 사이트 운영 중(https://saesoon-flower.vercel.app, 전용 도메인은 안 쓰기로 함). 다국어·PayPal·호접난 식당 배송·승진 토퍼·개인정보 동의·헤더 [예약 조회] 버튼·완료 화면 "예약 전 안내 문자" 안내 포함 |
| 예약 저장 (Supabase) | ✅ 운영 중. **DB 예약 4건 — 전부 취소 상태, 실제 손님 예약 0건**(21:05 확인): 9AF9D2A5(Claude PDF 시험, 메모 있음) · 0213E2AE(사용자 "래인2" 카톡 시험, 메모 있음) · E4A75539·7E13BBEE(메모 없음 — 시험 예약인지 사용자 확인 전). 예전 시험 예약들은 사용자 요청으로 삭제했었음 |
| 관리자 페이지 `/admin` | ✅ 사용자 로그인·상태 변경 사용 중 |
| 텔레그램 새 예약 알림 | ✅ 사장님 계정 2개(Kim·래인) 수신 확인 |
| 손님 예약 조회·취소 `/check` | ✅ 예약자 이름+연락처로 조회(접수번호 칸 없음), 취소 규칙대로 동작. 2026-09-15 21:00 **해외 번호 끝자리에 띄어쓰기·하이픈이 있으면 못 찾던 문제 수정**(`findReservationsByPhoneEnding` 정규식) — 실제 DB로 확인, 배포함 |
| 견적서·거래명세표 | ✅ 이메일 없이 PDF 다운로드만. 실제 사이트 PC 크롬에서 다운로드·한글·직인 확인. ⏳ 휴대폰·관리자 화면 다운로드는 직접 못 눌러봄 |
| 카카오톡으로 나에게 보내기 | ✅ 완료. 카톡 메시지 버튼 [예약 조회·취소]+[새순 예약하기], 사용자 휴대폰 확인 |
| 노션 날짜별 표 | ⚠️ 코드 배포됨, 실제 올리기 시험은 사용자가 원치 않음 → 첫 실제 예약을 "입금·결제 확인"할 때 확인 |
...
`NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY`(Config) · `SUPABASE_SECRET_KEY`(Secret) · `ADMIN_EMAILS`(Config) · `TELEGRAM_BOT_TOKEN`(Secret) · `TELEGRAM_CHAT_ID`(Config) · `NOTION_TOKEN`(Secret) · `NOTION_PARENT_PAGE_ID`(Config) · `NEXT_PUBLIC_KAKAO_JS_KEY`(Config, 카카오 버튼용 공개 키). `.env.local`·Vercel 모두 9개 있음(Vercel 노션 2개는 직접 못 봄).
...
2. **사용자 확인 대기**: 휴대폰(아이폰 Safari·카톡 안 브라우저)과 관리자 상세에서 PDF 다운로드 눌러보기 / 메모 없는 취소 예약 E4A75539·7E13BBEE가 시험 예약인지.
3. **점검에서 나온 것 (사용자 미결정, 2026-09-15 21:00)**:
   - 조회·취소 보안: 이름+연락처만 알면 남이 조회·입금 전 취소 가능, 틀린 시도 제한 약함(실패 0.8초 지연뿐) → 취소 때만 접수번호 확인 / 반복 실패 잠시 차단 제안.
   - 예약 전 안내 문자를 매장이 챙길 장치 없음(자동 발송 없음, 해외 번호는 문자 안 갈 수 있음) → 전날 텔레그램 "내일 예약 목록" / 관리자 "문자 보냄" 체크 / 외국어 문구를 "연락드려요"로 제안.
   - 작은 것: 취소된 예약도 PDF 주소를 직접 부르면 받아짐 · 카톡 보내기는 완료 화면에만 있음(조회 화면엔 없음) · 외국어(베트남어·영어) 헤더에서 가게 이름이 많이 잘릴 수 있음.
4. **노션 첫 실제 확인** — 첫 실제 예약을 "입금·결제 확인"으로 바꿀 때 상태 버튼 아래 안내 확인. "노션 연결 설정이 아직 없어서…"가 뜨면 Vercel에 `NOTION_TOKEN`/`NOTION_PARENT_PAGE_ID` 넣고 재배포.
5. **처리방침 내용 확인 권유** — 일반 양식임(보관 기간 "상품 전달 후 1년" 등). 사용자 확인 전.
6. **제안만 해둔 것 (사용자 미결정)**: PayPal 이메일 칸 이름을 "결제할 PayPal 이메일"로 짧게 / 직원도 알림 받기.
7. `C:\Users\plant\Desktop\래인\flower-reservation` 예전 복사본 — **그대로 둠**(사용자가 판단 맡김, 지운 사진 원본 포함·되돌릴 수 없어서). 필요 없으면 사용자가 직접 삭제.
8. 온라인 결제·이메일 발송·문자 자동 발송은 요청 전(이메일은 안 하기로 결정).
