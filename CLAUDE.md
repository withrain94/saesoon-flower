...
| 새 예약 알림 | **텔레그램**(2026-09-15 사용자 선택). 예약 저장 성공 뒤 매장 텔레그램으로: 접수번호·일시(특별한 날)·상품·금액·결제·**예약자 이름·연락처**(사용자 결정)·호접난 배송·토퍼/서류 여부·외국어 신청·관리자 링크. 실패해도 예약은 저장됨. 환경변수 `TELEGRAM_BOT_TOKEN`(비밀)·`TELEGRAM_CHAT_ID`(쉼표로 여러 곳). 처리방침 위탁에 Telegram 추가. 봇 **@saesoon_reservation_bot**, 받는 곳 = 사장님(Kim R) 채팅 — `.env.local`·Vercel(Production)에 둘 다 넣음(2026-09-15), 채팅 번호 8138762410. 배포 4b5a1fa → 실제 사이트 테스트 예약 C8B26952 저장(취소 처리). ✅ **실제 사이트 텔레그램 알림 도착 사용자 확인(2026-09-15) — 알림 기능 완료**. 직원 추가 = 봇에게 말 건 뒤 getUpdates로 채팅 번호 찾아 쉼표로 추가 | `server/notify.ts`, `lib/reservationNotice.ts` |
...
0. **노션 날짜별 예약 표 (2026-09-15 03:00, 코드 완성·커밋 안 됨·가짜 노션으로만 시험)** — 사용자 결정: **입금·결제 확인(이후 상태 포함)** 을 누르면 받는 날짜 페이지(`인재개발원 26/10/30` / `예약 26/10/15`)의 표에 상품 1개당 한 줄, 칸 = 사진 9칸 + 승진 토퍼·결제 방법·호접난 받는 방법·접수번호·매장 메모, 취소하면 예약종류 앞 `[취소]`. 코드: `lib/notionReservation.ts`, `server/notion.ts`(REST API 직접 호출, 부모 페이지 자식에서 제목으로 페이지 찾기·접수번호 칸으로 줄 찾기), `actions/admin.ts`, 상세 화면 "노션에 다시 올리기". 처리방침 5번에 Notion 추가. **남은 일: 사용자에게 노션 연결(integration) 만들기·부모 페이지 공유·`NOTION_TOKEN`/`NOTION_PARENT_PAGE_ID`를 .env.local·Vercel에 넣기 안내 → 실제 노션으로 확인 → 배포.** 줄은 입금 확인한 순서대로 붙음(시간순 정렬 아님). **인재개발원 날짜 페이지 특별요청(사용자)**: 표 아래에 `주문자 연락처 (중복 제외 N개)` / `받는분 연락처 (중복 제외 N개)` 소제목 + 번호 20개씩 `,`로 이은 문단(국내 번호 010-0000-0000 모양으로 맞춰 중복 판단, **취소 줄 제외**, 없으면 `(없음)`). 표가 바뀔 때마다 표 바로 아래에 새로 씀 — 번호만 적힌 문단만 지우므로 사장님 메모는 남음 (`hasContactLists`, `buildContactLists`, `refreshContactLists`). 가짜 노션 시험 통과. ✅ (2026-09-15 03:25) 실제 노션 연결 확인(읽기만): integration "새순 예약"(워크스페이스 새순), 부모 페이지 "새순 예약 표"(id 3dbd66b966dc80708335e6d95fc59537) 접근 OK, .env.local 설정 완료. Vercel에 NOTION_TOKEN(Secret)·NOTION_PARENT_PAGE_ID(Config) 넣으라고 안내 → 사용자 "배포해줘" → 7484197 배포. **실제 노션 올리기 시험은 사용자가 원치 않음 — 첫 실제 예약을 입금·결제 확인할 때 사용자가 노션 확인**. Vercel에 노션 값이 실제로 들어갔는지는 못 봄
