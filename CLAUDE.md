...
- **완료 화면 카카오톡으로 나에게 보내기·복사 (배포 완료 1a41e12)** — Vercel에 `NEXT_PUBLIC_KAKAO_JS_KEY`(Config)를 넣고 재배포해야 카카오 버튼이 보임. 없으면 복사 버튼만. ✅ 카카오 개발자 앱(이름 "새순") 만들고 JavaScript SDK 도메인에 배포 주소·localhost:3000 등록(사용자, 2026-09-15). ✅ `.env.local`에 키 넣고 로컬에서 카카오 공유 창 열림 확인(도메인 오류 없이 카카오 로그인 화면까지). ✅ 사용자가 Vercel에 키 넣고 대시보드에서 Redeploy(캐시 끔) → 실제 사이트 코드에 키 포함·실제 주소에서 카카오 공유 창 열림 확인(2026-09-15). ✅ 사용자가 휴대폰으로 시험 예약 → 노란 버튼으로 카톡 도착 확인(2026-09-15). 처음엔 카톡에 [예약 조회·취소] 버튼이 없었음 → 사용자가 카카오 앱 [앱]>[제품 링크 관리]>[웹 도메인]에 배포 주소 등록 → 다시 보내니 버튼 생김(사용자 확인 2026-09-15). **카카오 버튼 완료.** 사용자 요청(2026-09-15)으로 카톡 메시지 버튼 2개: [예약 조회·취소](/check) + [새순 예약하기](첫 화면) — 배포함, 휴대폰 확인 필요. 주소가 바뀌면(전용 도메인 등) JavaScript SDK 도메인·웹 도메인 두 곳 모두 추가해야 함. (참고: CLAUDE.md만 바꾼 커밋 deb2fa3은 Vercel이 배포를 만들지 않았음 — 원인 모름, 이럴 땐 대시보드 Redeploy)
